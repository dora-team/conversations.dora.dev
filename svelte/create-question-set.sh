...
LAYOUT_CSS_FILE="$ROUTE_DIR/layout.css"
...
<script>
    ...
    import { ${SLUG_UNDERSCORED}QuestionsUnrandomized } from "\$lib/index.js";
    import Conversation from "\$lib/Conversation.svelte";
</script>

<Conversation
    questionsUnrandomized={${SLUG_UNDERSCORED}QuestionsUnrandomized}
    showTimer={true}
...
# 4. Create a layout.css file for the new route
if [ -f "$LAYOUT_CSS_FILE" ]; then
  echo "Warning: File '$LAYOUT_CSS_FILE' already exists. Skipping file creation."
else
  echo "Creating file: $LAYOUT_CSS_FILE"
  cat <<EOF_LAYOUT_CSS > "$LAYOUT_CSS_FILE"
:global(body) {
  background-color: var(--dora-secondary-b); /* change as desired */
}
EOF_LAYOUT_CSS
  if [ $? -ne 0 ]; then echo "Error: Failed to create $LAYOUT_CSS_FILE"; exit 1; fi
fi

# 5. Create a new questions file
if [ -f "$QUESTIONS_FILE" ]; then
  echo "Warning: File '$QUESTIONS_FILE' already exists. Skipping file creation."
else
  echo "Creating file: $QUESTIONS_FILE"
  cat <<EOF_QUESTIONS > "$QUESTIONS_FILE"
What questions do you have?

EOF_QUESTIONS
  if [ $? -ne 0 ]; then echo "Error: Failed to create $QUESTIONS_FILE"; exit 1; fi
fi

# 6. Import and process the new questions in src/lib/index.js
IMPORT_LINE="import ${SLUG_UNDERSCORED}_questions_raw from \"../assets/${SLUG}-questions.txt?raw\";"
EXPORT_LINE="export const ${SLUG_UNDERSCORED}QuestionsUnrandomized = processQuestions(${SLUG_UNDERSCORED}_questions_raw);"
...
if grep -qF "$IMPORT_LINE" "$INDEX_JS_FILE"; then
  ...
  echo "" >> "$INDEX_JS_FILE" # Add a newline for separation
  echo "$IMPORT_LINE" >> "$INDEX_JS_FILE"
  echo "" >> "$INDEX_JS_FILE" # Add a newline for separation
  echo "$EXPORT_LINE" >> "$INDEX_JS_FILE"
